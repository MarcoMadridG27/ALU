...
wire [2:0] opcode; // <-- MODIFICADO (de [3:0] a [2:0])
    ...
    fpu_unit instanceFpu (
        .opcode(opcode), // Ahora conecta el opcode [2:0]
        .A1_d(A1_d),
        .A2_d(A2_d),
        .B1_d(B1_d),
        .B2_d(B2_d),
        ...
        .part_a(part_a),
        .part_b(part_b),
        .fpu_flags(fpu_flags) // Ahora conecta a la salida de 5 bits
    );
    ...
    // --- BLOQUE DE LOGS ---
    // Se ejecuta en cada ciclo de clk_slow
    always @(posedge clk) begin
        if (!reset) begin // No imprimir durante reset
            $display("-------------------------------------");
            $display("[%0t ns] Datapath @ State S%0d", $time, fsm_state);
            $display("  Opcode Latched: %b", opcode);
            $display("  FPU Output: part_a=%h, part_b=%h", part_a, part_b);
            $display("  Reg5 Output (Latched from prev cycle): part_ab=%h, part_bb=%h", part_ab, part_bb);
            $display("  Reg6 Output (Latched from prev cycle): part_bc=%h", part_bc);
            $display("  Mux Control: isResult=%b, partResult=%b", isResult, partResult);
            $display("  Mux Result (Final Output): result=%h", result);
            $display("  Flags Latched: %b", flags_save);
        end
    end
    // --- FIN BLOQUE DE LOGS ---
